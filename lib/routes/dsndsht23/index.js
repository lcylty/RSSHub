...
module.exports = async (ctx) => {
    ...
    const typefilter = ctx.params.type ? `&typeid=${ctx.params.type}&typeid=${ctx.params.type}&filter=typeid&page=1` : '';
	const typefilter2 = ctx.params.type ? `&typeid=${ctx.params.type}&typeid=${ctx.params.type}&filter=typeid&page=2` : '';
    const link = `${host}forum.php?mod=forumdisplay&fid=${subformId}${typefilter}`;
	const link2 = `${host}forum.php?mod=forumdisplay&fid=${subformId}${typefilter2}`;
    const headers = {
    ...
    const list2 = $1('#threadlisttableid tbody[id^=normalthread]')
        .slice(0, 11)
        .map(function () {
            const info = {
                title: '[' + $(this).find('th em a').text() + '] ' + $(this).find('a.xst').text(),
	...
	const listAll = [...list, ...list2];
    const out = await Promise.all(
        listAll.map(async (info) => {
